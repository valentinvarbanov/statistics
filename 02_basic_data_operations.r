...
set.seed(104)
N <- 30		#	Искаме да създадем data frame със симулирани 30 наблюдения.
...
boys_dating_diary <- data.frame(
    asking_for_dating_DATE = as.Date(Sys.time()) - N:1,			#	Дати
    girl_names = sample(x = girl_names, size = N, replace = T),		#	Стрингове
    accept = sample(x = c(T, F), size = N, replace = T, prob = c(0.1, 0.9)),	#	Булева променлива
    rating = round(runif(N, 1, 10))	#	Числа
) 

#	N:1 - създава се вектор с числа от N до 1 (намаляващ вариационен ред)
#	sample(x, size, repalce, prob) - на случаен принцип се избират N на брой елементи от предварително зададен вектор
...
head(x = boys_dating_diary, n = 3)	#	n - колко наблюдения искаме да видим. По подразбиране, стойността на n е 6
...
boys_dating_diary[1, ]	#	Първи ред и всички колони (индексът за колоните е празен)

boys_dating_diary[, 2]	#	Втора колона и всички редове (индексът за редовете е празен)
boys_dating_diary[1:nrow(boys_dating_diary), 2]	#	еквивалентен на горния ред

boys_dating_diary[5, c(2, 3)]	#	Пети ред, втора и трета колона
boys_dating_diary[5, 4]	#	5 ред и 4-та колона
...
#   - по колони
boys_dating_diary$girl_names		#	С "$" взимаме елементи от листа. Data frame-а е ЛИСТ от ВЕКТОРИ => взимаме вектора
boys_dating_diary[, "girl_names"]
#   Еквивалентни са на boys_dating_diary[, 2]


#   - по редове
boys_dating_diary["try_1", ]
#   Еквивалентхно е на boys_dating_diary[1, ]
...
#       Нека да проверим кои от всички 20 опита са се оказали сполучиливи. Тоест, 
#   приели са поканата на момчето
which(boys_dating_diary$accept)
boys_dating_diary[which(boys_dating_diary$accept), ]	#	Извеждаме самите наблюдения
#		boys_dating_diary$accept е булева променлива => which(boys_dating_diary$accept) ще ни върне индексите 
#	на наблюденията, които изпълняват това условие => имайки индексите с boys_dating_diary[индексите, ], 
#	извеждаме и самите резултати
boys_dating_diary[which(boys_dating_diary$accept), "girl_names"] #	Имената на момичетата, които са приели

boys_dating_diary[which(boys_dating_diary$rating >= 7), ]   #   Момичета, които са с рейтинг поне 7
#		Векторът boys_dating_diary$rating съдържа стойностите 9, 4, 2, 8, 3, 5, ..., 9, 6, 2, 4, 9, 10, 6, 9.
#	С условието boys_dating_diary$rating >= 7 ще върне T, F, F, T, F, F, ..., T, F, F, F, T, T, F, T =>
#	which(boys_dating_diary$rating >= 7) ще върне вектор с индекси 1, 4, 8, 9, 15, 16, 17, 23, 27, 28, 30 =>
#	boys_dating_diary[which(boys_dating_diary$rating >= 7), ] ще върне информацията за момичетата, които имат рейтинг поне 7
...
#       Как да вземем имената на момичетата, с които въпросният индивид се е опитал да се свърже
#   през последната седмица? Дали са приели?
